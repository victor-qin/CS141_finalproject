...
void tilemm(float * a, float * b, float * c, int out_rows, int out_cols, int in_cols, int tiles){
  ...
  //Divide matrix and fee to naivemm
  //add into main matrix
...
int main() {
  ...
  int MATRIX_ROWS = 1024;
  int MATRIX_COLS = MATRIX_ROWS; // for a square matrix
  clock_t start_clock, end_clock;
  double cpu_time;
  int number_of_iterations = 1; // TODO: Set this for consistent results
  int TILES = 64;
  // for(int TILES = 2; TILES <= 16; TILES *= 2) {
  float * mat_a_naive = generate_output(MATRIX_ROWS, MATRIX_ROWS);
  float * mat_a_tile = generate_output(MATRIX_ROWS, MATRIX_ROWS);
  ...
  for (int iter = 0; iter < number_of_iterations; iter++) {
    // mat_a_tile = generate_output(MATRIX_ROWS, MATRIX_ROWS);
    // mat_a_naive = generate_output(MATRIX_ROWS, MATRIX_ROWS);
    // tilemm(mat_a_tile, mat_b, mat_c, MATRIX_ROWS, MATRIX_COLS, MATRIX_COLS, TILES);
...
// needed.
//
// print_mat(mat_a_naive, MATRIX_ROWS, MATRIX_COLS);
// printf("\n");
// print_mat(mat_a_tile, MATRIX_ROWS, MATRIX_COLS);
...
// also correct.
// if ( compare_matrices(mat_a_naive, mat_a_tile, MATRIX_ROWS, MATRIX_COLS) ) {
